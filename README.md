...
A full-stack inventory, order, and customer management platform. FastAPI + PostgreSQL backend, a premium React/Tailwind frontend with light/dark themes and a command palette, all containerized with Docker.
...
| **Docker Hub image** | [hub.docker.com/r/aryannn5/nexstock-backend](https://hub.docker.com/r/aryannn5/nexstock-backend) |
| **GitHub repo** | [github.com/Aryan457dwivedi/NexStock](https://github.com/Aryan457dwivedi/NexStock) |

> The backend runs on Render's free tier, which sleeps after 15 minutes of inactivity. If the app looks slow or empty on first load, give it 30-60 seconds and refresh — that's the backend waking up, not a bug.
...
### Or pull the backend image directly

```bash
docker pull aryannn5/nexstock-backend:latest
docker run -p 8000:8000 -e DATABASE_URL=<your-postgres-url> aryannn5/nexstock-backend:latest
```
...
This project is deployed with **Render** (backend + database), **Vercel** (frontend), and published to **Docker Hub**.
...
4. Deploy — Render provides a URL like `https://nexstock-backend-plff.onrender.com`
...
**Note:** if Vercel's import screen offers to configure this as a multi-service project (because it also sees the `backend/` folder), explicitly set **Root Directory** to `frontend` before deploying — this project does not use Vercel's Services feature, since the backend runs independently on Render.

### Docker Hub (backend image)

Already published at [hub.docker.com/r/aryannn5/nexstock-backend](https://hub.docker.com/r/aryannn5/nexstock-backend). To rebuild and republish after changes:

```bash
docker build -t aryannn5/nexstock-backend:latest ./backend
docker push aryannn5/nexstock-backend:latest
```

## Author

**Aryan Dwivedi** — [github.com/Aryan457dwivedi](https://github.com/Aryan457dwivedi)
